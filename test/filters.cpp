...
TEST_CASE("Filter: escape") {
...

  {
    auto rendered = liquidpp::render(
        R"({{ "Have you read 'James & the Giant Peach'?" | escape }})", c);
...
TEST_CASE("Filter: size") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ "Ground control to Major Tom." | size }})", c);
    REQUIRE(rendered == "28");
  }
...
TEST_CASE("Filter: date") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({{ article.published_at | date: "%a, %b %d, %y" }})", c);
    REQUIRE(rendered == "Fri, Jul 17, 15");
  }

  {
    auto rendered = liquidpp::render(
        R"({{ article.published_at1 | date: "%a, %b %d, %y" }})", c);
    REQUIRE(rendered == "Fri, Jul 17, 15");
  }

  {
    auto rendered = liquidpp::render(
        R"({{ article.published_at2 | date: "%a, %b %d, %y" }})", c);
    REQUIRE(rendered == "Fri, Jul 17, 15");
  }

  {
    auto rendered =
        liquidpp::render(R"({{ article.published_at | date: "%Y" }})", c);
    REQUIRE(rendered == "2015");
  }

  {
    auto rendered =
        liquidpp::render(R"({{ "March 14, 2016" | date: "%b %d, %y" }})", c);
    REQUIRE(rendered == "Mar 14, 16");
  }

  {
    auto rendered = liquidpp::render(
        R"(This page was last updated at {{ "now" | date: "%Y-%m-%d %H:%M" }}.)",
        c);
    std::regex rx("This page was last updated at "
                  "[0-9]{4}-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9].");
...
TEST_CASE("Filter: upcase") {
...

  {
    ...
    CAPTURE(rendered);
    REQUIRE((rendered == u8"GRÜSSEN" || rendered == u8"GRÜßEN"));
  }
}
...
TEST_CASE("Filter: capitalize") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ "my great title" | capitalize }})", c);
    REQUIRE(rendered == "My great title");
  }

  {
    auto rendered = liquidpp::render(
        u8"{{ \"ähnlichkeit ist rein zufällig\" | capitalize }}", c);
...
TEST_CASE("Filter: append") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ "/my/fancy/url" | append: ".html" }})", c);
...
TEST_CASE("Filter: default") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ product_price | default: 2.99 }})", c);
    REQUIRE(rendered == "2.99");
  }

  {
    auto rendered = liquidpp::render(
        R"({% assign product_price = 4.99 %}{{ product_price | default: 2.99 }})",
        c);
  ...
  {
    auto rendered = liquidpp::render(
        R"({% assign product_price = "" %}{{ product_price | default: 2.99 }})",
        c);
    REQUIRE(rendered == "2.99");
  }
}
...
TEST_CASE("Filter: escape_once") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ "1 &lt; 2 &amp; 3" | escape_once }})", c);
    REQUIRE(rendered == "1 &lt; 2 &amp; 3");
  }
}
...
TEST_CASE("Filter: prepend") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({{ "apples, oranges, and bananas" | prepend: "Some fruit: " }})", c);
...
TEST_CASE("Filter: replace_first") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({% assign my_string = "Take my protein pills and put my helmet on" %}
...
TEST_CASE("Filter: strip_html") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({{ "Have <em>you</em> read <!--Ulysses-->?" | strip_html }})", c);
...
TEST_CASE("Filter: truncate") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({{ "Ground control to Major Tom." | truncate: 20 }})", c);
  ...
  {
    auto rendered =
        liquidpp::render(u8R"({{ "äöüßÄÖÜ" | truncate: 5, "" }})", c);
...
TEST_CASE("Filter: truncatewords") {
  ...
  {
    auto rendered = liquidpp::render(
        R"({{ "Ground control to Major Tom." | truncatewords: 3 }})", c);
    REQUIRE(rendered == "Ground control to...");
  }
...
TEST_CASE("Filter: url_encode") {
  ...
  {
    auto rendered =
        liquidpp::render(R"({{ "john@liquid.com" | url_encode }})", c);
    REQUIRE(rendered == "john%40liquid.com");
  }

  {
    auto rendered =
        liquidpp::render(R"({{ "Tetsuro Takara" | url_encode }})", c);
    REQUIRE(rendered == "Tetsuro+Takara");
  }
}
...
TEST_CASE("Filter: split") {
  ...
  {
    auto templ =
        R"({% assign beatles = "John, Paul, George, Ringo" | split: ", " %}
...
TEST_CASE("Filter: first") {
  ...
  {
    auto templ =
        R"({% assign my_array = "apples, oranges, peaches, plums" | split: ", " %}
...
apples)";
  ...
  {
    auto templ =
        R"({% assign my_array = "zebra, octopus, giraffe, tiger" | split: ", " %}
...
TEST_CASE("Filter: sort") {
  ...
  {
    auto templ =
        R"({% assign my_array = "zebra, octopus, giraffe, Sally Snake" | split: ", " %}
...
plums, peaches, oranges, apples)";
  ...
  {
    auto templ =
        R"({{ "Ground control to Major Tom." | split: "" | reverse | join: "" }})";
    auto rendered = liquidpp::render(templ, c);
...
TEST_CASE("Filter: uniq") {
  ...
  {
    auto templ =
        R"({% assign my_array = "ants, bugs, bees, bugs, ants" | split: ", " %}
